...
public class VMController implements APPlatformController {
	...
	public InstanceDescription createInstance(ProvisioningSettings settings) throws APPlatformException {
		...
		try {
			...
			if (platformService.exists(Controller.ID, id.getInstanceId())) {
				logger.error("Other instance with same name already registered in CTMG: [" + id.getInstanceId() + "]");
				throw new APPlatformException(Messages.getAll("error_instance_exists", id.getInstanceId()));
			}

			validateParameters(null, ph, settings.getOrganizationId(), id.getInstanceId());

			logger.info("createInstance({})", LogAndExceptionConverter.getLogText(id.getInstanceId(), settings));

			StateMachine.initializeProvisioningSettings(settings, "create_vm.xml");
			return id;
		} catch (Exception e) {
			throw LogAndExceptionConverter.createAndLogPlatformException(e, Context.CREATION);
		}
	}
	...
	public InstanceStatus deleteInstance(String instanceId, ProvisioningSettings settings) throws APPlatformException {
		...
		try {
			StateMachine.initializeProvisioningSettings(settings, "delete_vm.xml");
			InstanceStatus result = new InstanceStatus();
			result.setChangedParameters(settings.getParameters());
			return result;
		} catch (Exception e) {
			throw LogAndExceptionConverter.createAndLogPlatformException(e, Context.DELETION);
		}
	}
	...
	public InstanceStatus modifyInstance(String instanceId, ProvisioningSettings currentSettings,
		...
		try {
			VMPropertyHandler currentParameters = new VMPropertyHandler(currentSettings);
			VMPropertyHandler newParameters = new VMPropertyHandler(newSettings);

			validateParameters(currentParameters, newParameters, currentSettings.getOrganizationId(), instanceId);
			...
			StateMachine.initializeProvisioningSettings(newParameters.getSettings(), "modify_vm.xml");

			InstanceStatus result = new InstanceStatus();
			result.setChangedParameters(newSettings.getParameters());
			return result;
		} catch (Exception e) {
			throw LogAndExceptionConverter.createAndLogPlatformException(e, Context.MODIFICATION);
		}
	}
	...
	public InstanceStatus getInstanceStatus(String instanceId, ProvisioningSettings settings)
		...
		logger.debug("{}", LogAndExceptionConverter.getLogText(instanceId, settings));
		...
		try {
			...
			throw new SuspendException("Failed to get instance status for instance " + instanceId,
					new APPlatformException(t.getMessage()));
		}
	}

	private void updateProvisioningSettings(VMPropertyHandler ph, StateMachine stateMachine, String instanceId)
			throws Exception {

		String nextState = stateMachine.getStateId();
		switch (nextState) {
		case "REPEAT_FAILED_STATE":
			String failedState = stateMachine.loadPreviousStateFromHistory(ph.getProvisioningSettings());
			...
			platformService.storeServiceInstanceDetails(Controller.ID, instanceId, ph.getProvisioningSettings(),
					cred.toPasswordAuthentication());
			String errorMessage = ph.getServiceSetting(VMPropertyHandler.SM_ERROR_MESSAGE);
			throw new SuspendException(errorMessage);
		case "ERROR":
			errorMessage = ph.getServiceSetting(VMPropertyHandler.SM_ERROR_MESSAGE);
			throw new SuspendException(errorMessage);
		default:
			ph.setSetting(VMPropertyHandler.SM_STATE_HISTORY, stateMachine.getHistory());
	 ...
	 */
	private void validateParameters(VMPropertyHandler currentParameters, VMPropertyHandler newParameters,
			String customerOrgId, String instanceId) throws APPlatformException {
	...
	void validateDataDiskMountPoints(VMPropertyHandler newParameters) throws APPlatformException {

		for (String mointPointKey : newParameters.getDataDiskMountPointParameterKeys()) {

			String mountPoint = newParameters.getServiceSetting(mointPointKey);
			String validationPattern = newParameters.getMountPointValidationPattern(mointPointKey);
			...
			if (mountPoint == null || !mountPoint.matches(validationPattern)) {
				logger.debug("Invalid data disk mount point: {}, {}, {}", mointPointKey, mountPoint, validationPattern);
				throw new APPlatformException(Messages.getAll("error_invalid_data_disk_mount_point", mointPointKey,
						mountPoint, validationPattern));
			}
		}
	}

	private void validateMemory(VMPropertyHandler newParameters) throws APPlatformException {

		long memory = newParameters.getConfigMemoryMB();
		if (memory % 4 != 0) {
			logger.debug("Validation error on memory size [" + memory + "MB]");
			throw new APPlatformException(Messages.getAll("error_invalid_memory", Long.valueOf(memory)));
		}
	}

	private void validateDataDiskSizeReduction(VMPropertyHandler currentParameters, VMPropertyHandler newParameters)
			throws APPlatformException {
		...
		if (oldDataDisksMB.length > newDataDisksMB.length) {
			logger.warn("Reducing the number of data disks is not possible. instanceId: "
					+ currentParameters.getInstanceName() + " old number: " + currentParameters.getDataDisksMB().length
			...
			for (int i = 0; i < oldDataDisksMB.length; i++) {
				...
				if (dataDiskMB.longValue() > newDataDisksMB[i].longValue()) {
					...
					logger.error("Data disk size reduction is not possible. instanceId: "
							+ currentParameters.getInstanceName() + " old size: "
							+ currentParameters.getConfigDiskSpaceMB() + " new size: "
		...
		if (diskSizeReduction) {
			throw new APPlatformException(Messages.getAll("error_invalid_diskspacereduction"));
		}
	}

	private void validateSystemDiskSizeReduction(VMPropertyHandler currentParameters, VMPropertyHandler newParameters)
			throws APPlatformException {
		if (currentParameters.getConfigDiskSpaceMB() > newParameters.getConfigDiskSpaceMB()) {
			logger.error("System disk size reduction is not possible. old size: "
					+ currentParameters.getConfigDiskSpaceMB() + " new size: " + newParameters.getConfigDiskSpaceMB());
			throw new APPlatformException(Messages.getAll("error_invalid_diskspacereduction"));
		}
	}

	private void validateWindowsSettings(VMPropertyHandler params) throws APPlatformException {

		boolean isDomainJoin = params.isServiceSettingTrue(VMPropertyHandler.TS_WINDOWS_DOMAIN_JOIN);
		boolean domainName = params.getServiceSetting(VMPropertyHandler.TS_DOMAIN_NAME) != null;
		boolean admin = params.getServiceSetting(VMPropertyHandler.TS_WINDOWS_DOMAIN_ADMIN) != null;
		boolean adminPwd = params.getServiceSetting(VMPropertyHandler.TS_WINDOWS_DOMAIN_ADMIN_PWD) != null;
		logger.debug("isDomainJoin: " + isDomainJoin);

		if (isDomainJoin && !domainName) {
			throw new APPlatformException(Messages.getAll("error_missing_domain"));
		}
		if (isDomainJoin && !admin) {
			throw new APPlatformException(Messages.getAll("error_missing_domain_admin"));
		}
		if (isDomainJoin && !adminPwd) {
			throw new APPlatformException(Messages.getAll("error_missing_domain_admin_pwd"));
		}
	}
	...
	public InstanceStatus notifyInstance(String instanceId, ProvisioningSettings settings, Properties properties)
		...
		try {
			if ("finish".equals(properties.get("command"))) {
				...
				status.setChangedParameters(settings.getParameters());
				logger.debug(
						"Received finish event. Instance provisioning will be continued for instance " + instanceId);
			}
			return status;
		} catch (Throwable t) {
	...
	public InstanceStatus activateInstance(String instanceId, ProvisioningSettings settings)
		...
		try {
			StateMachine.initializeProvisioningSettings(settings, "activate_vm.xml");
			InstanceStatus result = new InstanceStatus();
			result.setChangedParameters(settings.getParameters());
			return result;
		} catch (Throwable t) {
	...
	public InstanceStatus deactivateInstance(String instanceId, ProvisioningSettings settings)
		...
		try {
			StateMachine.initializeProvisioningSettings(settings, "deactivate_vm.xml");
			InstanceStatus result = new InstanceStatus();
			result.setChangedParameters(settings.getParameters());
			return result;
		} catch (Throwable t) {
	...
	public InstanceStatus executeServiceOperation(String userId, String instanceId, String transactionId,
		...
		try {
			...
			case OPERATION_RESTART:
				StateMachine.initializeProvisioningSettings(settings, "restart_vm.xml");
				break;
			case OPERATION_START:
				StateMachine.initializeProvisioningSettings(settings, "start_vm.xml");
				break;
			case OPERATION_STOP:
				StateMachine.initializeProvisioningSettings(settings, "stop_vm.xml");
				break;
			case OPERATION_SNAPSHOT:
				StateMachine.initializeProvisioningSettings(settings, "snapshot_vm.xml");
				break;
			case OPERATION_RESTORE:
				StateMachine.initializeProvisioningSettings(settings, "restore_vm.xml");
				break;
			}
			InstanceStatus result = new InstanceStatus();
			result.setChangedParameters(settings.getParameters());
			...
			logger.debug("Failed to execute service operation " + operationId + " for instance " + instanceId
					+ " and user " + userId);
	...
	/**
	 * Get first available VCenter configuration
	 */
	private VCenter getVCenter(HashMap<String, Setting> controllerSettings) throws ConfigurationException {

		final List<VCenter> vCenters = getVCenterList(controllerSettings);
	 ...
	 */
	protected List<VCenter> getVCenterList(HashMap<String, Setting> controllerSettings) {
		ProvisioningSettings settings = new ProvisioningSettings(new HashMap<>(), controllerSettings,
	...
	private void cacheCredentials(VCenter vCenter) {
		cachedCredentials = new VMwareCredentials(vCenter.getUrl(), vCenter.getUserid(), vCenter.getPassword());
	}
