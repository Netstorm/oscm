...
public class OperatorSelectOrgBean extends BaseOperatorBean implements Serializable {
  ...
  public void setOrganizationId(String organizationId) {
    if (organizationId != null) {
      ...
      selectOrganizationActionIntern();
    }
  }

  public void selectOrganizationActionIntern() {
    try {
      reloadOrganization();
  ...
  public void selectOrganizationAction(AjaxBehaviorEvent event) {
    try {
      reloadOrganization();
    } catch (SaaSApplicationException exc) {
      ExceptionHandler.execute(exc);
  ...
  public List<Organization> getSuggestedOrgs() {
    List<Organization> organizations = new ArrayList<Organization>();
    Organization org = new Organization(new VOOrganization());
    org.setName("SomeName");
    org.setOrganizationId("PLATFORM_OPERATOR");
    org.setDescription("Some Description");

    Organization org2 = new Organization(new VOOrganization());
    org2.setName("SomeName2");
    org2.setOrganizationId("fef816bb");
    org2.setDescription("Some Description");
  ...
  public List<Organization> suggest(
      FacesContext context, UIComponent component, String organizationId) {
    organizationId = organizationId.replaceAll("\\p{C}", "");
    if (context == null || component == null) {
      return null;
    }
    Vo2ModelMapper<VOOrganization, Organization> mapper =
        new Vo2ModelMapper<VOOrganization, Organization>() {
          @Override
    ...
    try {
      ...
      if (!isBlank(value)) {
        try {
          ...
          while (st.hasMoreElements()) {
            roleTypes.add(OrganizationRoleType.valueOf(st.nextToken()));
          }
      ...
      List<Organization> organizations =
          mapper.map(getOperatorService().getOrganizations(pattern, roleTypes));
